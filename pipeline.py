...
'''
# none of the categoricals are missing, I guess it is safe to impute with mean for the others
>>> test_csv = pd.read_csv("test_.csv")
>>> test_csv.count()['x13']
2000
>>> test_csv.count()['x68']
...
2000
'''
test_csv.fillna(test_csv.mean(), inplace=True) # imputing for test data only

test_csv_dims = test_csv.shape
print("Shape of the test csv file is: ", test_csv_dims) # 8000 x 101
# label_test_csv
...
X_ = pd.get_dummies(data=cat_test_csv, drop_first=True)
